---
title: "sales_index_b2020_general_all_income"
...
df_lmb2020 <- sales_data_mod_b2020%>%filter(outlier==0)%>%select(filter_columns)
...
```{r}
df_lmb2020_opt <- sales_data_mod_b2020%>%filter(outlier==0)%>%filter(var_zip!="_11249")%>%select(c(sel_columns_lmb2020[!grepl("var_zip|^month|*Intercept*",sel_columns_lmb2020)],var_zip,month,zillow_sales_value_index,zillow_month_day))

lmb2020_opt <- lm(zillow_sales_value_index ~ .-zillow_month_day, data=df_lmb2020_opt)
