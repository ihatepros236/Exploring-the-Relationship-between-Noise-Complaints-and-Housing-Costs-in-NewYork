...
df_lma2020_low <- sales_data_mod_a2020%>%filter(outlier==0)%>%filter(income_group=="low")%>%select(filter_columns)
lma2020_low <- lm(zillow_sales_value_index ~ ., data=df_lma2020_low)
...
```{r}
df_lma2020_low_opt <- sales_data_mod_a2020%>%filter(outlier==0)%>%filter(income_group=="low")%>%select(c(sel_columns_lma2020_low[!grepl("var_zip|^month|*Intercept*",sel_columns_lma2020_low)],var_zip,month,zillow_sales_value_index,zillow_month_day))

lma2020_low_opt <- lm(zillow_sales_value_index ~ .-zillow_month_day, data=df_lma2020_low_opt)
...
filter_columns = list_columns[grepl("^comp_ratio*|^zillow_sales_value_index|var_zip|^month", list_columns)]
filter_columns = filter_columns[!filter_columns %in% c("month_number","-zillow_sales_value_index_lag")]
df_lma2020_medium <- sales_data_mod_a2020%>%filter(outlier==0)%>%filter(income_group=="medium")%>%select(filter_columns)
lma2020_medium <- lm(zillow_sales_value_index ~ ., data=df_lma2020_medium)
...
```{r}
df_lma2020_medium_opt <- sales_data_mod_a2020%>%filter(outlier==0)%>%filter(income_group=="medium")%>%select(c(sel_columns_lma2020_medium[!grepl("var_zip|^month|*Intercept*",sel_columns_lma2020_medium)],var_zip,month,zillow_sales_value_index,zillow_month_day))

lma2020_medium_opt <- lm(zillow_sales_value_index ~ .-zillow_month_day, data=df_lma2020_medium_opt)
...
filter_columns = list_columns[grepl("^comp_ratio*|^zillow_sales_value_index|var_zip|^month", list_columns)]
filter_columns = filter_columns[!filter_columns %in% c("month_number","-zillow_sales_value_index_lag")]
df_lma2020_high <- sales_data_mod_a2020%>%filter(outlier==0)%>%filter(income_group=="high")%>%select(filter_columns)
lma2020_high <- lm(zillow_sales_value_index ~ ., data=df_lma2020_high)
...
```{r}
df_lma2020_high_opt <- sales_data_mod_a2020%>%filter(outlier==0)%>%filter(income_group=="high")%>%select(c(sel_columns_lma2020_high[!grepl("var_zip|^month|*Intercept*",sel_columns_lma2020_high)],var_zip,month,zillow_sales_value_index,zillow_month_day))

lma2020_high_opt <- lm(zillow_sales_value_index ~ .-zillow_month_day, data=df_lma2020_high_opt)
