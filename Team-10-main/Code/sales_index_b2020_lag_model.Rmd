...
```{r}
list_columns <- colnames(sales_data_mod_b2020_lag)
...
df_lmb2020_low_lag <- sales_data_mod_b2020_lag%>%filter(outlier==0)%>%filter(income_group=="low")%>%filter(!is.na(zillow_sales_value_index_lag))%>%select(filter_columns)
lmb2020_low_lag <- lm(zillow_sales_value_index_lag ~ .-zillow_zip, data=df_lmb2020_low_lag)  
...
```{r}
df_lmb2020_low_lag_opt <- sales_data_mod_b2020_lag%>%filter(outlier==0)%>%filter(income_group=="low")%>%filter(!is.na(zillow_sales_value_index_lag))%>%select(c(sel_columns_lmb2020_low_lag[!grepl("var_zip|^month|*Intercept*",sel_columns_lmb2020_low_lag)],var_zip,month,zillow_sales_value_index_lag,zillow_month_day_lag))

lmb2020_low_lag_opt <- lm(zillow_sales_value_index_lag ~ . -zillow_zip, data=df_lmb2020_low_lag_opt%>%select(!zillow_month_day_lag))
...
dfpred_lmb2020_low_lag_opt <- sales_data_mod_b2020_lag%>%filter(outlier==0)%>%filter(income_group=="low")%>%select(c(sel_columns_lmb2020_low_lag[!grepl("var_zip|^month|*Intercept*",sel_columns_lmb2020_low_lag)],var_zip,month,zillow_sales_value_index_lag,zillow_month_day_lag))
...
ggplot(data = dfpred_lmb2020_low_lag_opt%>%select(zillow_month_day_lag, prediction, zillow_sales_value_index_lag),aes(x = as.Date(zillow_month_day_lag))) + 
...
filter_columns = list_columns[grepl("^comp_ratio*|^zillow_sales_value_index|var_zip|^month", list_columns)]
filter_columns = filter_columns[!filter_columns %in% c("month_number","zillow_sales_value_index")]
df_lmb2020_medium_lag <- sales_data_mod_b2020_lag%>%filter(outlier==0)%>%filter(income_group=="medium")%>%filter(!is.na(zillow_sales_value_index_lag))%>%select(filter_columns)
lmb2020_medium_lag <- lm(zillow_sales_value_index_lag ~ .-zillow_zip, data=df_lmb2020_medium_lag)  
...
```{r}
df_lmb2020_medium_lag_opt <- sales_data_mod_b2020_lag%>%filter(outlier==0)%>%filter(income_group=="medium")%>%filter(!is.na(zillow_sales_value_index_lag))%>%select(c(sel_columns_lmb2020_medium_lag[!grepl("var_zip|^month|*Intercept*",sel_columns_lmb2020_medium_lag)],var_zip,month,zillow_sales_value_index_lag,zillow_month_day_lag))

lmb2020_medium_lag_opt <- lm(zillow_sales_value_index_lag ~ . -zillow_zip, data=df_lmb2020_medium_lag_opt%>%select(!zillow_month_day_lag))
...
dfpred_lmb2020_medium_lag_opt <- sales_data_mod_b2020_lag%>%filter(outlier==0)%>%filter(income_group=="medium")%>%filter(var_zip != "_11366")%>%select(c(sel_columns_lmb2020_medium_lag[!grepl("var_zip|^month|*Intercept*",sel_columns_lmb2020_medium_lag)],var_zip,month,zillow_sales_value_index_lag,zillow_month_day_lag))
...
ggplot(data = dfpred_lmb2020_medium_lag_opt%>%select(zillow_month_day_lag, prediction, zillow_sales_value_index_lag),aes(x = as.Date(zillow_month_day_lag))) + 
  stat_summary(aes(y = prediction, colour = "Forecasted Index (n+4)"),geom = "line", fun = "mean", size= 1) +
  stat_summary(aes(y = prediction),geom = "ribbon", fun.data = "mean_cl_normal", alpha = 0.1,fill = "red") +
  stat_summary(aes(y=zillow_sales_value_index_lag, colour = "Actual Index"), geom = "line", fun = "mean", size= 1) +
  stat_summary(aes(y = zillow_sales_value_index_lag),geom = "ribbon", fun.data = "mean_cl_normal", alpha = 0.1,fill = "blue") +
...
filter_columns = list_columns[grepl("^comp_ratio*|^zillow_sales_value_index|var_zip|^month", list_columns)]
filter_columns = filter_columns[!filter_columns %in% c("month_number","zillow_sales_value_index")]
df_lmb2020_high_lag <- sales_data_mod_b2020_lag%>%filter(outlier==0)%>%filter(income_group=="high")%>%filter(!is.na(zillow_sales_value_index_lag))%>%select(filter_columns)
lmb2020_high_lag <- lm(zillow_sales_value_index_lag ~ .-zillow_zip, data=df_lmb2020_high_lag)  
...
```{r}
df_lmb2020_high_lag_opt <- sales_data_mod_b2020_lag%>%filter(outlier==0)%>%filter(income_group=="high")%>%filter(!is.na(zillow_sales_value_index_lag))%>%select(c(sel_columns_lmb2020_high_lag[!grepl("var_zip|^month|*Intercept*",sel_columns_lmb2020_high_lag)],var_zip,month,zillow_sales_value_index_lag,zillow_month_day_lag))

lmb2020_high_lag_opt <- lm(zillow_sales_value_index_lag ~ . -zillow_zip, data=df_lmb2020_high_lag_opt%>%select(!zillow_month_day_lag))
...
dfpred_lmb2020_high_lag_opt <- sales_data_mod_b2020_lag%>%filter(outlier==0)%>%filter(income_group=="high")%>%select(c(sel_columns_lmb2020_high_lag[!grepl("var_zip|^month|*Intercept*",sel_columns_lmb2020_high_lag)],var_zip,month,zillow_sales_value_index_lag,zillow_month_day_lag))
...
ggplot(data = dfpred_lmb2020_high_lag_opt%>%select(zillow_month_day_lag, prediction, zillow_sales_value_index_lag),aes(x = as.Date(zillow_month_day_lag))) + 
  stat_summary(aes(y = prediction, colour = "Forecasted Index (n+4)"),geom = "line", fun = "mean", size= 1) +
  stat_summary(aes(y = prediction),geom = "ribbon", fun.data = "mean_cl_normal", alpha = 0.1,fill = "red") +
  stat_summary(aes(y=zillow_sales_value_index_lag, colour = "Actual Index"), geom = "line", fun = "mean", size= 1) +
  stat_summary(aes(y = zillow_sales_value_index_lag),geom = "ribbon", fun.data = "mean_cl_normal", alpha = 0.1,fill = "blue") +
